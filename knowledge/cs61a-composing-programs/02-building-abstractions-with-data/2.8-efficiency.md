...
```javascript
// Define fib so recursive calls go through the wrapper
let fib = function(n) {
  if (n === 0) return 0;
  if (n === 1) return 1;
  return fib(n - 2) + fib(n - 1);
...
// 5702887
fibCallCount;
// 15  (only new cache misses; values 0-19 were already cached from fib(19))
```
...
function fastExp(b, n) {
  ...
  if (n % 2 === 0) {
    return square(fastExp(b, Math.floor(n / 2)));
  } else {
...
Note: JavaScript `Number` uses IEEE 754 double-precision, so very large integer results lose precision. For exact integer arithmetic with large exponents, use `BigInt`: `fastExp(2n, 100n)` would yield the exact value, but requires adjusting the function to use `BigInt` arithmetic.

**Note on integer division**: In Python, `n // 2` performs integer (floor) division. In JavaScript, `Math.floor(n / 2)` is the standard approach.

The process evolved by `fastExp` grows logarithmically with `n` in both space and number of steps. To see this, observe that computing `b^(2n)` using `fastExp` requires only one more multiplication than computing `b^n`. The size of the exponent we can compute therefore doubles (approximately) with every new multiplication we are allowed. Thus, the number of multiplications required for an exponent of `n` grows about as fast as the logarithm of `n` base 2. The process has `Theta(log n)` growth. The difference between `Theta(log n)` growth and `Theta(n)` growth becomes striking as `n` becomes large. For example, `fastExp` for `n` of 1000 requires only 16 recursive calls instead of 1000.

We can verify this difference using call counting:

```javascript
let myExp = undefined;
myExp = count(function exp(b, n) {
  if (n === 0) return 1;
  return b * myExp(b, n - 1);
});

let myFastExp = undefined;
myFastExp = count(function fastExp(b, n) {
  if (n === 0) return 1;
  if (n % 2 === 0) {
    return square(myFastExp(b, Math.floor(n / 2)));
  } else {
    return b * myFastExp(b, n - 1);
  }
});
...
myFastExp.callCount = 0;  // reset counter
myFastExp(2, 1000);
...
| **Integer division** | `n // 2` | `Math.floor(n / 2)` (bit shift `n >> 1` is limited to 32-bit signed integers) |
