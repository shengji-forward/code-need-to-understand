...
const bundle = readFileSync(resolve(here, 'preface-tldraw-map.bundle.js'), 'utf8')
...
function toolbar({ nav }) {
  ...
  <span class="title">Preface tldraw story deck</span>
  <span class="hint">${hint}</span>
  ${navControls}
  <button onclick="saveDiagram()">Save .tldr</button>
  <button onclick="loadDiagram()">Load .tldr</button>
  <button onclick="exportSvg()">Export SVG</button>
  <button onclick="resetPrefaceStory()">Reset template</button>
</div>`
}
...
<div id="root"></div>
<script>
window.PREFACE_TLDRAW_VARIANT = ${JSON.stringify(variant)};
window.PREFACE_TLDRAW_NAV = ${JSON.stringify(nav)};
</script>
<script>
${bundle}
</script>
...
const outputs = [
  {
    file: 'slides-tldraw-map.html',
    title: 'Preface tldraw story deck - nav',
    nav: true,
    variant: 'nav',
  },
  {
  ...
  },
  {
    ...
    title: 'Preface tldraw story deck - plain',
    nav: false,
    variant: 'plain',
  },
]
